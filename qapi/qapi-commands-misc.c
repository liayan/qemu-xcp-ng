...
static void qmp_marshal_output_XenPvDriverInfo(XenPvDriverInfo *ret_in, QObject **ret_out, Error **errp)
{
    Error *err = NULL;
    Visitor *v;

    v = qobject_output_visitor_new(ret_out);
    visit_type_XenPvDriverInfo(v, "unused", &ret_in, &err);
    if (!err) {
        visit_complete(v, ret_out);
    }
    error_propagate(errp, err);
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_XenPvDriverInfo(v, "unused", &ret_in, NULL);
    visit_free(v);
}

void qmp_marshal_query_xen_platform_pv_driver_info(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    XenPvDriverInfo *retval;
    Visitor *v = NULL;
    ...
    retval = qmp_query_xen_platform_pv_driver_info(&err);
    if (err) {
        goto out;
    }

    qmp_marshal_output_XenPvDriverInfo(retval, ret, &err);
...
void qmp_marshal_query_migratable(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    Visitor *v = NULL;
