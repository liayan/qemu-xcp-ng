...
void qapi_event_send_xen_platform_pv_driver_info(int64_t product_num, int64_t build_num)
{
    ...
    QObject *obj;
    Visitor *v;
    q_obj_XEN_PLATFORM_PV_DRIVER_INFO_arg param = {
        product_num, build_num
    };

    qmp = qmp_event_build_dict("XEN_PLATFORM_PV_DRIVER_INFO");

    v = qobject_output_visitor_new(&obj);

    visit_start_struct(v, "XEN_PLATFORM_PV_DRIVER_INFO", NULL, 0, &error_abort);
    visit_type_q_obj_XEN_PLATFORM_PV_DRIVER_INFO_arg_members(v, &param, &error_abort);
    visit_check_struct(v, &error_abort);
    visit_end_struct(v, NULL);

    visit_complete(v, &obj);
    qdict_put_obj(qmp, "data", obj);
    qapi_event_emit(QAPI_EVENT_XEN_PLATFORM_PV_DRIVER_INFO, qmp);
